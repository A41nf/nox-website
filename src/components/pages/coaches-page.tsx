        ...
        {coaches.map((coach, index) => (
          <Reveal key={coach.name} delay={index * 0.08}>
            <article className="h-full rounded-[2rem] border border-white/10 bg-white/[0.04] p-7">
              ...
              <p className="mt-5 text-base leading-8 text-white/68">{coach.bio}</p>
              <div className="mt-5 flex flex-wrap gap-2">
                {coach.specialties.map((specialty) => (
                  <span key={specialty} className="rounded-full border border-white/10 px-3 py-1 text-xs text-white/65">
                    {specialty}
                  </span>
                ))}
              </div>
              <blockquote className="mt-6 border-r-2 border-[#E80028] pr-4 text-sm leading-7 text-white/60">
                {coach.quote}
              </blockquote>
            </article>
          </Reveal>
